...
ERC20Processor.bind(
    { address: DERIVE_VAULTS.SUSDEBULL_MAINNET.destinationChainAddress, network: EthChainId.ETHEREUM }
)
    .onEventTransfer(async (event, ctx) => {
        for (const user of [event.args.from, event.args.to]) {
            let [oldSnapshot, newSnapshot] = await vaults.updateVaultUserSnapshot(ctx, DERIVE_VAULTS.SUSDEBULL_MAINNET, ctx.address, user, [], pools.swellL2.getSwellL2Balance)
            emitVaultUserPoints(ctx, DERIVE_VAULTS.SUSDEBULL_MAINNET, oldSnapshot, newSnapshot)
        }
    })
...
ERC20Processor.bind(
    { address: DERIVE_VAULTS.SUSDEBULL_ARB.destinationChainAddress, network: EthChainId.ARBITRUM }
)
    .onEventTransfer(async (event, ctx) => {
        for (const user of [event.args.from, event.args.to]) {
            let [oldSnapshot, newSnapshot] = await vaults.updateVaultUserSnapshot(ctx, DERIVE_VAULTS.SUSDEBULL_ARB, ctx.address, user, [], pools.swellL2.getSwellL2Balance)
            emitVaultUserPoints(ctx, DERIVE_VAULTS.SUSDEBULL_ARB, oldSnapshot, newSnapshot)
        }
    })
    // this time interval handles all three vaults (weETHC, weETHCS, weETHBULL)
    .onTimeInterval(async (_, ctx) => {
...
ERC20Processor.bind(
    { address: DERIVE_VAULTS.SUSDEBULL_BASE.destinationChainAddress, network: EthChainId.BASE }
)
    .onEventTransfer(async (event, ctx) => {
        for (const user of [event.args.from, event.args.to]) {
            let [oldSnapshot, newSnapshot] = await vaults.updateVaultUserSnapshot(ctx, DERIVE_VAULTS.SUSDEBULL_BASE, ctx.address, user, [], pools.swellL2.getSwellL2Balance)
            emitVaultUserPoints(ctx, DERIVE_VAULTS.SUSDEBULL_BASE, oldSnapshot, newSnapshot)
        }
    })
    // this time interval handles all three vaults (weETHC, weETHCS, weETHBULL)
    .onTimeInterval(async (_, ctx) => {
...
GlobalProcessor.bind(
    ...
    await saveCurrentVaultTokenPrice(ctx as EthContext, DERIVE_VAULTS.SUSDEBULL_BASE)
},
    60 * 24,
    60 * 24
)
