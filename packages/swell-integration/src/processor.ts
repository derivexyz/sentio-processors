...
ERC20Processor.bind(
    { address: DERIVE_VAULTS.RSWETHC_MAINNET.destinationChainAddress, network: EthChainId.ETHEREUM }
)
    .onEventTransfer(async (event, ctx) => {
        for (const user of [event.args.from, event.args.to]) {
            let [oldSnapshot, newSnapshot] = await vaults.updateVaultUserSnapshot(ctx, DERIVE_VAULTS.RSWETHC_MAINNET, ctx.address, user, [VAULT_POOLS.SWELL_L2.address], pools.swellL2.getSwellL2Balance)
            emitVaultUserPoints(ctx, DERIVE_VAULTS.RSWETHC_MAINNET, oldSnapshot, newSnapshot)
        }
    })
...
GlobalProcessor.bind(
    ...
    await saveCurrentVaultTokenPrice(ctx, DERIVE_VAULTS.RSWETHC_MAINNET)
},
    60 * 24,
    60 * 24
)
...
const rsweth_filter = SubaccountsProcessor.filters.BalanceAdjusted(null, null, V2_ASSETS["RSWETH"].assetAndSubId, null, null, null, null)
SubaccountsProcessor.bind(
    ...
    .onEventBalanceAdjusted(async (event, ctx) => {
        await v2.snapshot.updateExchangeBalance(ctx, V2_ASSETS["RSWETH"], event.args.accountId, event.args.postBalance.scaleDown(18), excludedSubaccounts, emitUserExchangePoints)
    }, rsweth_filter)
